...
1. 异： 
   ...
   1. 定义的不同，一个使用 `@staticmethod`, 一个使用 `@classmethod`.

   2. 参数不同。静态方法的参数只包含方法本身需要的参数。而类方法的第一个参数是固定的 `cls`,表示类对象（类的元数据对象）。

   3. **类方法用在模拟java定义多个构造函数的情况**。 由于Python类中只能有一个初始化方法，不能按照不同的情况初始化类。

   4. 静态方法是类中的函数，不需要实例。静态方法主要是用来存放逻辑性的代码，主要是一些逻辑属于类，但是和类本身没有交互，即在静态方法中，不会涉及到类中的方法和属性的操作。可以理解为将静态方法存在此类的名称空间中。在静态方法中，不会涉及到类中的属性和方法的操作
   ...
   6. 类方法是将类本身作为对象进行操作的方法。他和静态方法的区别在于：不管这个方式是从实例调用还是从类调用，它都用第一个参数把类传递过来**。**

2. 同：
   1. 调用方法相同，都是 Class.method()或 instance.method()
...
- has-a关系通常称之为**关联**，比如部门和员工的关系，汽车和引擎的关系都属于关联关系；关联关系如果是整体和部分的关联，那么我们称之为**聚合**关系；如果整体进一步负责了部分的生命周期（整体和部分是不可分割的，同时同在也同时消亡），那么这种就是最强的关联关系，我们称之为**合成**关系。
